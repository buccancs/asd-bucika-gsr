...
class ChartLogView : LineChart {
 ...
 this.setPinchZoom(false)//xy
 this.isDoubleTapToZoomEnabled = false//
 this.isScaleYEnabled = false//Y
 this.isScaleXEnabled = true//X
 ...
 xAxis.gridColor = axisChartColors //x
 xAxis.axisLineColor = 0x00000000 //x
 ...
 leftAxis.textColor = textColor //y
 leftAxis.axisLineColor = 0x00000000 //y
 leftAxis.setDrawGridLines(true)//
 leftAxis.gridColor = axisChartColors //y
            leftAxis.gridLineWidth = 1.5f
            leftAxis.setLabelCount(6, true)
 leftAxis.valueFormatter = YValueFormatter()//settings
...
//                    data[0].type = "default"
                    when (data[0].type) {
                        "point" -> {
 var set = lineData.getDataSetByIndex(0)//x0
                            if (set == null) {
 ...
 Log.w("123", "")
                        ...
                        "line" -> {
 var maxDataSet = lineData.getDataSetByIndex(0)//x0
                            if (maxDataSet == null) {
                                maxDataSet = createSet(0, "line max temp")

                            }

 var minDataSet = lineData.getDataSetByIndex(1)//x0
                            if (minDataSet == null) {
...
//                                Log.w("123", "x: $x")
                            ...
                            //max
 var maxTempDataSet = lineData.getDataSetByIndex(0)//x0
                            if (maxTempDataSet == null) {
                                maxTempDataSet = createSet(0, "fence max temp")
                                lineData.addDataSet(maxTempDataSet)
                            }
                            //center
 var centerTempDataSet = lineData.getDataSetByIndex(1)//x0
 ...
 setVisibleXRangeMinimum(ChartTools.getMinimum(type = type) / 2)//settingsX
 setVisibleXRangeMaximum(ChartTools.getMaximum(type = type))//settingsX
...
//                    ChartTools.setY(this@ChartTempView)
 ...
 set.fillDrawable = ContextCompat.getDrawable(context, bgChartColors[index])//settings
