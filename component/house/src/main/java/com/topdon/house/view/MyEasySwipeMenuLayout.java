...
public class MyEasySwipeMenuLayout extends ViewGroup {
    ...
    public MyEasySwipeMenuLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MyEasySwipeMenuLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            for (int i = 0; i < indexCount; i++) {
                ...
                if (attr == R.styleable.MyEasySwipeMenuLayout_leftMenuView) {
                    mLeftViewResID = typedArray.getResourceId(R.styleable.MyEasySwipeMenuLayout_leftMenuView, -1);
                } else if (attr == R.styleable.MyEasySwipeMenuLayout_rightMenuView) {
                    mRightViewResID = typedArray.getResourceId(R.styleable.MyEasySwipeMenuLayout_rightMenuView, -1);
                } else if (attr == R.styleable.MyEasySwipeMenuLayout_contentView) {
                    mContentViewResID = typedArray.getResourceId(R.styleable.MyEasySwipeMenuLayout_contentView, -1);
                } else if (attr == R.styleable.MyEasySwipeMenuLayout_canLeftSwipe) {
                    mCanLeftSwipe = typedArray.getBoolean(R.styleable.MyEasySwipeMenuLayout_canLeftSwipe, true);
                } else if (attr == R.styleable.MyEasySwipeMenuLayout_canRightSwipe) {
                    mCanRightSwipe = typedArray.getBoolean(R.styleable.MyEasySwipeMenuLayout_canRightSwipe, true);
                } else if (attr == R.styleable.MyEasySwipeMenuLayout_fraction) {
                    mFraction = typedArray.getFloat(R.styleable.MyEasySwipeMenuLayout_fraction, 0.5f);
                }
            }
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                maxWidth = Math.max(maxWidth,
                        child.getMeasuredWidth() + lp.leftMargin + lp.rightMargin + getPaddingStart() + getPaddingEnd());
                maxHeight = Math.max(maxHeight,
                        child.getMeasuredHeight() + lp.topMargin + lp.bottomMargin + getPaddingTop() + getPaddingBottom());
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (lp.width == LayoutParams.MATCH_PARENT) {
                    final int width = Math.max(0, getMeasuredWidth()
                            - lp.leftMargin - lp.rightMargin - getPaddingStart() - getPaddingEnd());
